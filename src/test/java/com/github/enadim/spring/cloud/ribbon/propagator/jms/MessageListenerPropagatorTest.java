...
public class MessageListenerPropagatorTest {
    MessageListener delegate = mock(MessageListener.class);
    MessageListenerPropagator propagator = new MessageListenerPropagator(delegate, new HashSet<>());
    Message message = mock(Message.class);

    @Test
    public void onMessage() throws Exception {
        when(message.getPropertyNames()).thenReturn(Collections.emptyEnumeration());
        propagator.onMessage(message);
