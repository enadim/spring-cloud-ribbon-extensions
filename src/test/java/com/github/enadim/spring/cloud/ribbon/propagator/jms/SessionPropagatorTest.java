...
public class SessionPropagatorTest {
    Set<String> keys = new HashSet<>(asList("1"));
    Session delegate = mock(Session.class);
    SessionPropagator propagator = new SessionPropagator(delegate, keys);
    ...
    public void setMessageListener() throws Exception {
        ...
        reset(delegate);
        propagator.setMessageListener(new MessageListenerPropagator(mock(MessageListener.class), null));
        verify(delegate).setMessageListener(any(MessageListenerPropagator.class));
    }
