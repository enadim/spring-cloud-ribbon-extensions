...
public class AbstractOnMessagePropagatorTest {
    private Set<String> keysToPropagate = new HashSet<>(asList("1", "2"));
    private AbstractOnMessagePropagator propagator = new AbstractOnMessagePropagator(keysToPropagate) {
    };
    ...
    public void copy_matching_properties() throws Exception {
        ...
        propagator.processMessage(message);
        verify(message, never()).getStringProperty(eq("3"));
        keysToPropagate.forEach(x -> Assert.assertThat(current().get(x), is(x)));
    ...
    public void fail_on_get_property_names() throws Exception {
        ...
        propagator.processMessage(message);
        Assert.assertThat(current().getAttributes().keySet(), empty());
    }
    ...
    public void fail_on_get_property() throws Exception {
        ...
        Assert.assertThat(current().getAttributes().size(), is(1));
    }
