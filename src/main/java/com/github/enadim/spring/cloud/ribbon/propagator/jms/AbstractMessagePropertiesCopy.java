...
/**
 * Copies message properties to the current {@link ExecutionContext} pre-filtering property names using the provided {@link #filter}.
 * <p>Defines the utility method {@link #copyFromMessage(Message)} to be used by implementation
 */
@Slf4j
@Getter
public abstract class AbstractMessagePropertiesCopy {
    /**
     ...
     */
    public AbstractMessagePropertiesCopy(@NotNull Filter<String> filter) {
        this.filter = filter;
    }

    /**
     * Copies message properties to the current {@link ExecutionContext}.
     *
