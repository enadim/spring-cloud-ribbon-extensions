...
public class ExecutionContextAwareCallable<T> implements Callable<T> {
     ...
     */
    public ExecutionContextAwareCallable(Callable<T> delegate) {
    ...
    /**
     * Wraps a callable to a {@link ExecutionContextAwareCallable}
     *
     * @param callable the callable to wrap
     * @param <T>      the callable result type
     * @return The {@link ExecutionContextAwareCallable} instance over #callable
     */
    public static <T> ExecutionContextAwareCallable<T> wrap(Callable<T> callable) {
        return new ExecutionContextAwareCallable<>(callable);
    }

    /**
     * Wraps the a callable's collection to a {@link ExecutionContextAwareCallable} collection.
     *
     * @param tasks the callable's collection to wrap
     * @param <T>   the callable result type
     * @return The {@link ExecutionContextAwareCallable} instances over #tasks
     */
    public static <T> Collection<? extends Callable<T>> wrap(Collection<? extends Callable<T>> tasks) {
        return tasks.stream().map(ExecutionContextAwareCallable::wrap).collect(Collectors.toList());
    }
}
