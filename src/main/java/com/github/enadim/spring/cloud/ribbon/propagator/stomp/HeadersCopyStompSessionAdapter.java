 ...
 */
@Slf4j
public class HeadersCopyStompSessionAdapter extends AbstractExecutionContextCopy<StompHeaders> implements StompSession {
    /**
     ...
     */
    public HeadersCopyStompSessionAdapter(@NotNull StompSession delegate, @NotNull Filter<String> filter) {
        super(filter, StompHeaders::set);
    ...
    public Subscription subscribe(String destination, StompFrameHandler handler) {
        return delegate.subscribe(destination, new HeadersCopyStompFrameHandlerAdapter(handler, getFilter()));
    }
    ...
    public Subscription subscribe(StompHeaders headers, StompFrameHandler handler) {
        return delegate.subscribe(headers, new HeadersCopyStompFrameHandlerAdapter(handler, getFilter()));
    }
